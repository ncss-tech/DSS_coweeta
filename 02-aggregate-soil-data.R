...
## TODO: move this to 01-prepare-soil-data.R

## mukey grids for area calculation
s.grid <- rast('grids/ssurgo_utm.tif')
r.grid <- rast('grids/rss_utm.tif')

# area by cell value (mukey) 
s.area <- expanse(s.grid, byValue = TRUE)
r.area <- expanse(r.grid, byValue = TRUE)

# combine and clean-up
mu.area <- make.groups(ssurgo = s.area, rss = r.area)
mu.area$layer <- NULL
row.names(mu.area) <- NULL
names(mu.area) <- c('mukey', 'mu.area', 'which')


# check: ok
head(mu.area)
tapply(mu.area$mu.area, mu.area$which, sum)

# save a copy for later
saveRDS(mu.area, file = 'data/mu-area.rds')


# join with SPC
site(x) <- mu.area[, c('mukey', 'mu.area')]

# create a component pct & area weight
site(x)$co.wt <- x$mu.area * (x$comppct_r / 100)
...
# weights are mu area * component area fraction
a.site <- slab(y, fm = source ~ sandtotal_r + silttotal_r + claytotal_r + dbthirdbar_r + wthirdbar_r + wfifteenbar_r + ksat_r + ksat.rosetta, weights = 'co.wt')
...
## TODO: save this figure for later use
...
xyplot(
  ...
  lower = a.site$p.q25, upper = a.site$p.q75, 
  ylim = c(205,-5),
...
xyplot(top ~ p.q50 | variable, groups = source, data=a.site, ylab='Depth (cm)',
       subset = variable %in% c('Ksat [SSURGO] (cm/d)', 'Ksat [ROSETTA] (cm/d)'),
       xlab='median bounded by 25th and 75th percentiles',
       ...
       strip=strip.custom(bg=grey(0.85)),
       layout=c(2,1), scales=list(x=list(alternating=1, relation='free'), y=list(alternating=3)),
       par.settings = tps,
       as.table = TRUE,
       auto.key=list(columns=2, lines=TRUE, points=FALSE)
)
...
# a <- slab(
#   y, 
#   fm = cokey ~ sandtotal_r + silttotal_r + claytotal_r + dbthirdbar_r + wthirdbar_r + wfifteenbar_r + ksat_r, 
#   slab.fun = weighted.mean, na.rm = TRUE)
# 
# 
# # long -> wide format
