...
setwd("C:/Coweeta_Modeling/Combined_Results")
getwd()
shp.pts <-readOGR(dsn=".", layer="modeling_points")
...
comp <- comp[,-c(1)]  # removes the ID column
...
# subsed the raster stack to the selected covariates
...
comp.sub$Class<-make.names(comp.sub$Class)
...
#Alternative for all covariates
comp.sub<-comp
names(comp.sub)
...
beginCluster()
pred <- clusterR(r.stack, predict, args=list(model=rf.model),progress="text")
endCluster()
gc()
plot(pred)
...
predprob <- clusterR(r.stack, predict, args=list(model=rf.model, type="prob", index = 1:length(rf.model$classes)),progress="text")
...
setwd("C:/Coweeta_Modeling/Frigid_results/")
writeRaster(pred, overwrite = TRUE, filename = "comp_class_all.tif", options=c("COMPRESS=DEFLATE", "TFW=YES"),datatype='INT1U', progress="text")
# probability rasters
writeRaster(predprobint, overwrite=TRUE,filename="probmatrix_sf_all.tif", options=c("COMPRESS=DEFLATE", "TFW=YES"),datatype='INT1U', progress="text")
writeRaster(probmaxint, overwrite=TRUE,filename="probmax_all.tif", options=c("COMPRESS=DEFLATE", "TFW=YES"),datatype='INT1U', progress="text")
...
write.table(lookup_tab, file = "PredRATall.txt", sep = "\t", row.names = FALSE)
